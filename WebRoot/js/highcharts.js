...
(function () {
	...
	function Bd() {
		...
		Dc = a ? Date.UTC : function (b, c, d, e, f, g) {
			return (new Date(b, c, C(d, 1), C(e, 0), C(f, 0), C(g, 0))).getTime();
		};
	...
	function Gd(a, b) {
		function c(m, i) {
			...
			function Ma() {
				...
				if (F) {
					ta = [];
					A = Wa.global.useUTC;
					var G = 1000 / rb, V = 60000 / rb, Ba = 3600000 / rb;
					U = 86400000 / rb;
					k = 604800000 / rb;
					q = 2592000000 / rb;
					var ma = 31556952000 / rb, s = [["second", G, [1, 2, 5, 10, 15, 30]], ["minute", V, [1, 2, 5, 10, 15, 30]], ["hour", Ba, [1, 2, 3, 4, 6, 8, 12]], ["day", U, [1, 2]], ["week", k, [1, 2]], ["month", q, [1, 2, 3, 4, 6]], ["year", ma, null]], W = s[6], ba = W[1], ia = W[2];
					...
					if (ba >= G) {
						ia.setSeconds(ba >= V ? 0 : s * kb(ia.getSeconds() / s));
					}
					if (ba >= V) {
			...
			function Ra() {
				...
				if (q) {
					...
					if (ka) {
						...
						ka = ga.path(A).attr({stroke:i.lineColor, "stroke-width":q, zIndex:7}).add();
					}
				}
				if (v.axisTitle) {
					A = z ? Y : ha;
				...
				if (o && o.enabled) {
					...
					o = v.stackTotalGroup;
					if (!o) {
						v.stackTotalGroup = o = ga.g("stack-labels").attr({visibility:Ab, zIndex:6}).translate(Y, ha).add();
					}
					for (Ba in t) {
						k = t[Ba];
						for (ma in k) {
			...
			var id, Sb = [], Ta, Kc, Fb, ta, tb = {}, Zb = {}, fc = {}, hc, ic, gd, Lc, Xa = i.categories, $d = i.labels.formatter || function () {
				var k = this.value;
				return Lc ? Nc(Lc, k) : Ta % 1000000 === 0 ? k / 1000000 + "M" : Ta % 1000 === 0 ? k / 1000 + "k" : !Xa && k >= 1000 ? zd(k, 0) : k;
			...
			y.prototype = {addLabel:function () {
				...
				k = $d.call({isFirst:k === ta[0], isLast:k === ta[ta.length - 1], dateTimeLabelFormat:Lc, value:Xa && Xa[k] ? Xa[k] : k});
				q = q && {width:Ha(1, X(q - 2 * (o.padding || 10))) + bb};
				q = pa(q, o.style);
				if (A === Va) {
					this.label = M(k) && r && o.enabled ? ga.text(k, 0, 0).attr({align:o.align, rotation:o.rotation}).css(q).add(gc) : null;
				} else {
					A && A.attr({text:k}).css(q);
				}
			}, getLabelSize:function () {
				var k = this.label;
				return k ? (this.labelBBox = k.getBBox())[z ? "height" : "width"] : 0;
			}, render:function (k, o) {
				var r = !this.minor, q = this.label, A = this.pos, U = i.labels, G = this.gridLine, V = r ? i.gridLineWidth : i.minorGridLineWidth, Ba = r ? i.gridLineColor : i.minorGridLineColor, ma = r ? i.gridLineDashStyle : i.minorGridLineDashStyle, s = this.mark, W = r ? i.tickLength : i.minorTickLength, ba = r ? i.tickWidth : i.minorTickWidth || 0, ia = r ? i.tickColor : i.minorTickColor, zb = r ? i.tickPosition : i.minorTickPosition;
				r = U.step;
				var lb = o && Pc || Ua, Nb;
				...
				if (ba) {
					...
					if (s) {
						...
						this.mark = ga.path(V).attr({stroke:ia, "stroke-width":ba}).add(gc);
					}
				}
				if (q && !isNaN(Nb)) {
			...
			x.prototype = {render:function () {
				...
				if (ma) {
					if (s) {
						ma.animate({d:s}, null, ma.onGetPath);
					} else {
						ma.hide();
				...
				if (r && M(r.text) && s && s.length && Ca > 0 && xa > 0) {
					...
					if (!q) {
						k.label = q = ga.text(r.text, 0, 0).attr({align:r.textAlign || r.align, rotation:r.rotation, zIndex:ba}).css(r.style).add();
					}
					G = [s[1], s[4], C(s[6], s[1])];
					s = [s[2], s[5], C(s[7], s[2])];
					W = qb.apply(sa, G);
					ba = qb.apply(sa, s);
					q.align(r, false, {x:W, y:ba, width:Ha.apply(sa, G) - W, height:Ha.apply(sa, s) - ba});
					q.show();
				} else {
					q && q.hide();
				}
			...
			Q.prototype = {setTotal:function (k) {
				...
				if (this.label) {
					this.label.attr({text:o, visibility:ub});
				} else {
					this.label = m.renderer.text(o, 0, 0).css(this.options.style).attr({align:this.textAlign, rotation:this.options.rotation, visibility:ub}).add(k);
				}
			...
			pa(v, {addPlotBand:Qa, addPlotLine:Qa, adjustTickAmount:function () {
				...
				if (k || Rb) {
					u(ta, function (V) {
						...
						if (Z === 0 || Z === 2 || {1:"left", 3:"right"}[Z] === A.align) {
				...
				if (q && q.text) {
					if (!v.axisTitle) {
						v.axisTitle = ga.text(q.text, 0, 0).attr({zIndex:7, rotation:q.rotation || 0, align:q.textAlign || {low:"left", middle:"center", high:"right"}[q.align]}).css(q.style).add();
					}
					o = v.axisTitle.getBBox()[z ? "height" : "width"];
					r = C(q.margin, z ? 5 : 10);
				}
				P = U * (i.offset || Ub[Z]);
				gd = ic + (Z !== 2 && ic && U * i.labels[z ? "y" : "x"]) + r;
			...
			}, removePlotBand:Za, removePlotLine:Za, reversed:$b, stacks:t});
			for (id in N) {
				Sa(v, id, N[id]);
			}
			Aa();
		...
		function e(m) {
			function i() {
				...
				var $ = Pb(P) || H, oa;
				oa = $ ? ["<span style=\"font-size: 10px\">" + (H ? Nc("%A, %b %e, %Y", P) : P) + "</span>"] : [];
				u(F, function (ua) {
		...
		function f(m, i) {
			...
			Qc = function () {
				if (Rc) {
					...
					m.trackerGroup = Rc = ga.g("tracker").attr({zIndex:9}).add();
				}
			};
			Qc();
			if (i.enabled) {
						...
						if (nd && t.type !== "touchstart") {
							...
							if (Ma > 10) {
								if (lc && (Ra || Za) && kc(O - Y, ea - ha)) {
									Aa || (Aa = ga.rect(Y, ha, Ga ? 1 : Ca, Ka ? 1 : xa, 0).attr({fill:"rgba(69,114,167,0.25)", zIndex:7}).add());
								}
								if (Aa && Ga) {
									v = v - O;
							...
							if (!F) {
								...
								if (uc && i.shared) {
									...
									P = Ia.length;
									for ($ = 0; $ < P; $++) {
										if (Ia[$].visible && Ia[$].tooltipPoints.length) {
								...
								if (v && v.tracker) {
									(t = v.tooltipPoints[ua]) && t !== N && t.onMouseOver();
								}
							}
						}
						return (z = F) || !lc;
				...
				ya.onclick = function (t) {
					...
					if (!Ma) {
						if (v && Ea(t.target, "isTracker")) {
							...
							pa(v, {pageX:tc.left + Y + (qa ? Ca - F : N), pageY:tc.top + ha + (qa ? xa - N : F)});
		...
		function n(m, i) {
			...
			u([["title", m, Kb], ["subtitle", i, wc]], function (y) {
				...
				if (y && y.text && !Q) {
					p[x] = ga.text(y.text, 0, 0).attr({align:y.align, "class":"highcharts-" + x, zIndex:1}).css(y.style).add().align(y, false, Ob);
				}
			});
		}
		function J() {
		...
		function aa() {
			...
			if (!p.seriesGroup) {
				p.seriesGroup = ga.g("series-group").attr({zIndex:3}).add();
			}
			u(Ia, function (x) {
				x.translate();
				x.setTooltipPoints();
			...
			if (i.enabled && !p.credits) {
				...
				}).attr({align:i.position.align, zIndex:8}).css(i.style).add().align(i.position);
			}
			Qc();
			p.hasRendered = true;
			if (Vb) {
		...
		R = Lb(R) ? R : [R, R, R, R];
		var B = C(w.marginTop, R[0]), K = C(w.marginRight, R[1]), S = C(w.marginBottom, R[2]), I = C(w.marginLeft, R[3]), za = w.spacingTop, Da = w.spacingRight, gb = w.spacingBottom, wb = w.spacingLeft, Ob, Kb, wc, ha, Hb, sb, Y, Ub, mb, Vb, ya, Pd, Uc, xc, Ya, Ua, jd, Pc, Xc, ud, vd, Yc, p = this, ae = (R = w.events) && !!R.click, wd, kc, uc, nd, bc, Sd, xd, xa, Ca, jc, Rc, Qc, od, Wb, Xb, tc, lc = w.showAxes, Sc = 0, db = [], Mb, Ia = [], qa, ga, kd, Od, ld, pd, vc, sd, td, rd, md, Td, be = function (m) {
			function i(L, ka) {
			...
			function Q(L) {
				...
				if (!Ja) {
					...
					L.legendItem = Ja = ga.text(O.labelFormatter.call(L), 0, 0).css(L.visible ? Za : Ka).on("mouseover", function () {
						L.setState(Bb);
						Ja.css(Ga);
					...
					if (!Fa && Oa && Oa.lineWidth) {
						...
						L.legendLine = ga.path([ab, -Ma - Aa, 0, La, -Aa, 0]).attr(ca).add(fb);
					}
					if (Fa) {
						ka = ga.rect(T = -Ma - Aa, ra = -11, Ma, 12, 2).attr({zIndex:3}).add(fb);
					} else {
						if (Oa && Oa.marker && Oa.marker.enabled) {
							ka = ga.symbol(L.symbol, T = -Ma / 2 - Aa, ra = -4, Oa.marker.radius).attr({zIndex:3}).add(fb);
						}
					}
					if (ka) {
						ka.xOff = T + fa % 2 / 2;
					...
					if (Oa && Oa.showCheckbox) {
						...
						Sa(L.checkbox, "click", function (Gb) {
							Pa(L, "checkboxClick", {checked:Gb.target.checked}, function () {
								L.select();
							});
						});
					}
			...
			function na() {
				...
				u(Tb, function (ra) {
					var Fa = ra.options;
					if (Fa.showInLegend) {
						Qa = Qa.concat(Fa.legendType === "point" ? ra.data : ra);
					}
				});
				...
				if (oa || ua) {
					...
					Xb += 2 * z;
					if ($) {
						Wb > 0 && Xb > 0 && $.animate($.crisp(null, null, null, Wb, Xb));
					} else {
						$ = ga.rect(0, 0, Wb, Xb, O.borderRadius, oa || 0).attr({stroke:O.borderColor, "stroke-width":oa || 0, fill:ua || jb}).add(fb).shadow(O.shadow);
					}
					$[Qa.length ? "show" : "hide"]();
				}
		...
		vc = function () {
			...
			if ((p.title || p.subtitle) && !M(B)) {
				if (O = Ha(p.title && !Kb.floating && !Kb.verticalAlign && Kb.y || 0, p.subtitle && !wc.floating && !wc.verticalAlign && wc.y || 0)) {
		...
		pd = function () {
			...
			if (m || i) {
				if (Xc) {
					...
					Xc = ga.rect(Q / 2, Q / 2, Ya - Q, Ua - Q, w.borderRadius, m).attr({stroke:w.borderColor, "stroke-width":m, fill:i || jb}).add().shadow(w.shadow);
				}
			}
			if (y) {
				if (ud) {
					...
					ud = ga.rect(Y, ha, Ca, xa, 0).attr({fill:y}).add().shadow(w.plotShadow);
				}
			}
			if (x) {
				if (vd) {
			...
			if (w.plotBorderWidth) {
				if (Yc) {
					...
					Yc = ga.rect(Y, ha, Ca, xa, 0, w.plotBorderWidth).attr({stroke:w.plotBorderColor, "stroke-width":w.plotBorderWidth, zIndex:4}).add();
				}
			}
			p.isDirtyBox = false;
		};
		...
		p.showLoading = function (m) {
			var i = a.loading;
			if (!bc) {
				bc = ib(Qb, {className:"highcharts-loading"}, pa(i.style, {left:Y + bb, top:ha + bb, width:Ca + bb, height:xa + bb, zIndex:10, display:jb}), ya);
	...
	var wa = document, cb = window, sa = Math, X = sa.round, kb = sa.floor, fd = sa.ceil, Ha = sa.max, qb = sa.min, $a = sa.abs, nb = sa.cos, Db = sa.sin, cc = sa.PI, Ud = cc * 2 / 360, pc = navigator.userAgent, Bc = /msie/i.test(pc) && !cb.opera, zc = wa.documentMode === 8, Nd = /AppleWebKit/.test(pc), Rd = /Firefox/.test(pc), yc = !!wa.createElementNS && !!wa.createElementNS("http://www.w3.org/2000/svg", "svg").createSVGRect, Wc, Jb = wa.documentElement.ontouchstart !== undefined, Vd = {}, qd = 0, rb = 1, Hc, Wa, Nc, Cc, $c, Va, Qb = "div", oc = "absolute", Qd = "relative", ub = "hidden", nc = "highcharts-", Ab = "visible", bb = "px", jb = "none", ab = "M", La = "L", Wd = "rgba(192,192,192," + (yc ? 0.000001 : 0.002) + ")", hb = "", Bb = "hover", Dc, bd, cd, dd, sc, Ec, Fc, Cd, Dd, ed, Ed, Fd, la = cb.HighchartsAdapter, Eb = la || {}, u = Eb.each, yd = Eb.grep, mc = Eb.map, va = Eb.merge, Sa = Eb.addEvent, Cb = Eb.removeEvent, Pa = Eb.fireEvent, Zc = Eb.animate, Tc = Eb.stop, vb = {};
	Nc = function (a, b, c) {
		function d(D) {
		...
		n = n.months;
		b = {a:J[g].substr(0, 3), A:J[g], d:d(h), e:h, b:n[j].substr(0, 3), B:n[j], m:d(j + 1), y:l.toString().substr(2, 2), Y:l, H:d(f), I:d(f % 12 || 12), l:f % 12 || 12, M:d(b[bd]()), p:f < 12 ? "AM" : "PM", P:f < 12 ? "am" : "pm", S:d(b.getSeconds())};
		for (e in b) {
			a = a.replace("%" + e, b[e]);
		}
	...
	$c = {init:function (a, b, c) {
		...
		var h, j, l = function (n) {
			for (g = n.length; g--; ) {
				n[g] === ab && n.splice(g + 1, 0, n[g + 1], n[g + 2], n[g + 1], n[g + 2]);
			}
		};
		if (e) {
			l(b);
	...
	xb.column = va(la, {borderColor:"#FFFFFF", borderWidth:1, borderRadius:0, groupPadding:0.2, marker:null, pointPadding:0.1, minPointLength:0, states:{hover:{brightness:0.1, shadow:false}, select:{color:"#C0C0C0", borderColor:"#000000", shadow:false}}, dataLabels:{y:null, verticalAlign:null}});
			...
			for (c in a) {
				...
				if (c === "d") {
					...
					if (c === "x" && h === "text") {
						...
						if (this.rotation) {
							Ea(g, "transform", "rotate(" + this.rotation + " " + d + " " + ja(a.y || Ea(g, "y")) + ")");
						}
					} else {
						if (c === "fill") {
							d = j.color(d, g, c);
	...
	Vc.prototype = {Element:Ic, init:function (a, b, c, d) {
		...
		for (var b = a.element, c = C(a.textStr, "").toString().replace(/<(b|strong)>/g, "<span style=\"font-weight:bold\">").replace(/<(i|em)>/g, "<span style=\"font-style:italic\">").replace(/<a/g, "<span").replace(/<\/(b|strong|i|em|a)>/g, "</span>").split(/<br.*?>/g), d = b.childNodes, e = /style="([^"]+)"/, f = /href="([^"]+)"/, g = Ea(b, "x"), h = a.styles, j = Rd && h && h.HcDirection === "rtl" && !this.forExport && ja(pc.split("Firefox/")[1]) < 4, l, n = h && ja(h.width), J = h && h.lineHeight, D, aa = d.length; aa--; ) {
			b.removeChild(d[aa]);
		}
		...
		this.width = a;
		this.height = b;
		for (this.boxWrapper[C(c, true) ? "animate" : "attr"]({width:a, height:b}); e--; ) {
			d[e].align();
		}
	}, g:function (a) {
		...
		if (g) {
			...
			if (h.test(a)) {
				var l = function (n, J) {
					n.attr({width:J[0], height:J[1]}).translate(-X(J[0] / 2), -X(J[1] / 2));
				};
				j = a.match(h)[1];
				a = Vd[j];
		...
		var e = d.start, f = d.end - 0.000001, g = d.innerR, h = nb(e), j = Db(e), l = nb(f);
		f = Db(f);
		d = d.end - e < cc ? 0 : 1;
	...
	if (!yc) {
		Eb = yb(Ic, {init:function (a, b) {
				...
				for (c in a) {
					...
					if (l && /^(x|y|r|start|end|width|height|innerR)/.test(c)) {
						...
						if (c === "d") {
							...
							e = d.length;
							for (n = []; e--; ) {
								n[e] = dc(d[e]) ? X(d[e] * 10) - 5 : d[e] === "Z" ? "x" : d[e];
							}
							d = n.join(" ") || "x";
							f.path = d;
							if (D) {
			...
			if (this.added) {
				...
				if (b.tagName === "SPAN") {
					...
					var D = 0, aa;
					J = ja(a.textWidth);
					var E = a.xCorr || 0, da = a.yCorr || 0, w = [c, g, b.innerHTML, a.textWidth].join(",");
					if (w !== a.cTT) {
						if (M(c)) {
							l = c * Ud;
							d = nb(l);
		...
		la.prototype = va(Vc.prototype, {Element:Eb, isIE8:pc.indexOf("MSIE 8.0") > -1, init:function (a, b, c) {
			...
			var d = Wa.chart.style;
			return this.createElement("span").attr({text:a, x:X(b), y:X(c)}).css({whiteSpace:"nowrap", fontFamily:d.fontFamily, fontSize:d.fontSize});
		}, path:function (a) {
	...
	Ac.prototype = {init:function (a, b) {
		...
		if (!(a === this.state || this.selected && a !== "select" || c[a] && c[a].enabled === false || a && (f || e && !d.enabled))) {
			if (this.graphic) {
				...
				if (a) {
					if (!g) {
						b.stateMarkerGraphic = g = h.renderer.circle(0, 0, j[a].r).attr(j[a]).add(b.group);
					}
	...
	pb.prototype = {isCartesian:true, type:"line", pointClass:Ac, pointAttrToOptions:{stroke:"lineColor", "stroke-width":"lineWidth", fill:"fillColor", r:"radius"}, init:function (a, b) {
		...
		u(d, function (j, l) {
			...
			for (g = j._high = d[l + 1] ? kb((j.plotX + (d[l + 1] ? d[l + 1].plotX : e)) / 2) : e; f <= g; ) {
				h[c ? e - f++ : f++] = j;
			}
		});
		...
		if (this.options.marker.enabled) {
			for (f = b.length; f--; ) {
				...
				if (e !== Va && !isNaN(e)) {
					...
					h = a.r;
					if (j) {
						j.animate({x:d, y:e, r:h});
					} else {
						g.graphic = c.renderer.symbol(C(g.marker && g.marker.symbol, this.symbol), d, e, h).attr(a).add(this.group);
					}
				}
			}
		}
		...
		if (a.options.marker) {
			...
			d.color = d.color || Yb(d.color || f).brighten(d.brightness).get();
		}
		...
		for (f = h.length; f--; ) {
			...
			if (e) {
				...
				e = c[Bb] = c[Bb] || {};
				if (!a.options.marker) {
					e.color = Yb(e.color || g.options.color).brighten(e.brightness || d.brightness).get();
				}
		...
		if (this.options.dataLabels.enabled) {
			...
			if (J) {
				if (n) {
					if (D) {
						e = va(e, {verticalAlign:"middle"});
					}
					if (aa) {
						e = va(e, {y:{top:14, middle:4, bottom:-6}[e.verticalAlign]});
					}
				} else {
					if (D) {
						e = va(e, {verticalAlign:"top"});
					}
				}
			}
			if (!g) {
				g = a.dataLabelsGroup = h.renderer.g("data-labels").attr({visibility:a.visible ? Ab : ub, zIndex:6}).translate(h.plotLeft, h.plotTop).add();
			}
			n = e.color;
			if (n === "auto") {
				n = null;
			...
			u(d, function (E) {
				...
				if (B) {
					if (j && !e.y) {
						c = c + ja(B.styles.lineHeight) * 0.9 - B.getBBox().height / 2;
					}
					B.attr({text:f}).animate({x:b, y:c});
				} else {
					if (M(f)) {
						B = E.dataLabel = h.renderer.text(f, b, c).attr({align:K, rotation:e.rotation, zIndex:1}).css(e.style).add(g);
		...
		u(a.segments, function (w) {
			...
			if (aa) {
				...
				if (b.stacking && a.type !== "areaspline") {
					...
					R.push(La, w[w.length - 1].plotX, D, La, w[0].plotX, D);
				}
				da = da.concat(R);
			}
		});
		...
		a.singlePoints = E;
		if (aa) {
			e = C(b.fillColor, Yb(a.color).setOpacity(b.fillOpacity || 0.75).get());
			if (f) {
				f.animate({d:da});
			} else {
				a.area = a.chart.renderer.path(da).attr({fill:e}).add(g);
			}
		}
		if (c) {
			c.animate({d:d});
		...
		if (!h) {
			h = a.clipRect = !b.hasRendered && b.clipRect ? b.clipRect : j.clipRect(0, 0, b.plotSizeX, b.plotSizeY);
			if (!b.clipRect) {
				b.clipRect = h;
			}
		}
		...
		if (g) {
			...
			a.tracker = e.renderer.path(c).attr({isTracker:true, stroke:Wd, fill:jb, "stroke-width":b.lineWidth + 2 * f, visibility:a.visible ? Ab : ub, zIndex:1}).on(Jb ? "touchstart" : "mouseover", function () {
	...
	var ad = yb(pb, {type:"column", pointAttrToOptions:{stroke:"borderColor", "stroke-width":"borderWidth", fill:"color", r:"borderRadius"}, init:function () {
		...
		D = h * c.groupPadding;
		var aa = (h - 2 * D) / f, E = c.pointWidth, da = M(E) ? (aa - E) / 2 : aa * c.pointPadding, w = Ha(C(E, aa - 2 * da), 1), R = da + (D + ((g ? f - a.columnIndex : a.columnIndex) || 0) * aa - h / 2) * (g ? -1 : 1), B = a.yAxis.getThreshold(c.threshold || 0), K = C(c.minPointLength, 5);
		u(J, function (S) {
		...
		u(a.data, function (f) {
			...
			if (g !== Va && !isNaN(g) && f.y !== null) {
				...
				if (d) {
					...
					f.graphic = c[f.shapeType](e).attr(f.pointAttr[f.selected ? "select" : hb]).add(a.group).shadow(b.shadow);
				}
			}
		});
	}, drawTracker:function () {
		var a = this, b = a.chart, c = b.renderer, d, e, f = +new Date, g = a.options.cursor, h = g && {cursor:g}, j;
		u(a.data, function (l) {
			e = l.tracker;
			...
			if (l.y !== null) {
				if (e) {
					...
					l.tracker = c[l.shapeType](d).attr({isTracker:f, fill:Wd, visibility:a.visible ? Ab : ub, zIndex:1}).on(Jb ? "touchstart" : "mouseover", function (n) {
	...
	la = yb(pb, {type:"pie", isCartesian:false, pointClass:la, pointAttrToOptions:{stroke:"borderColor", "stroke-width":"borderWidth", fill:"color"}, getColor:function () {
		...
		g = mc(g, function (I, za) {
			return (R = /%$/.test(I)) ? [j, l, w, w][za] * ja(I) / 100 : I;
		});
		...
		u(aa, function (I) {
			...
			I.labelPos = [g[0] + B + nb(D) * S, g[1] + K + Db(D) * S, g[0] + B + nb(D) * f, g[1] + K + Db(D) * f, g[0] + B, g[1] + K, S < 0 ? "center" : D < E / 4 ? "left" : "right", D];
			I.percentage = da * 100;
			I.total = a;
		});
		...
		u(this.data, function (j) {
			...
			g = j.shadowGroup;
			if (f && !g) {
				g = j.shadowGroup = b.g("shadow").attr({zIndex:4}).add();
			}
			if (!e) {
				e = j.group = b.g("point").attr({zIndex:5}).add();
			}
			...
			if (d) {
				...
				j.graphic = b.arc(h).attr(pa(j.pointAttr[hb], {"stroke-linejoin":"round"})).add(j.group).shadow(f, g);
			}
			j.visible === false && j.setVisible(false);
		});
	}, drawDataLabels:function () {
		var a = this.data, b, c = this.chart, d = this.options.dataLabels, e = C(d.connectorPadding, 10), f = C(d.connectorWidth, 1), g, h, j = d.distance, l = this.radiusY, n = j > 0, J = this.center[1], D = [[], []], aa, E, da, w, R = 2, B;
		if (d.enabled) {
			...
			for (a = D[0][0] && D[0][0].dataLabel && ja(D[0][0].dataLabel.styles.lineHeight); R--; ) {
				...
				for (B = 0; B < za; B++) {
					...
					Da = E.i;
					E = E.y;
					if (aa > E && K[Da + 1] !== null || aa < E && K[Da - 1] !== null) {
						E = aa;
					}
					aa = this.getX(E, R);
					g.attr({visibility:da, align:h[6]})[g.moved ? "animate" : "attr"]({x:aa + d.x + ({left:e, right:-e}[h[6]] || 0), y:E + d.y});
					g.moved = true;
					if (n && f) {
						g = b.connector;
						...
						if (g) {
							...
							b.connector = g = this.chart.renderer.path(h).attr({"stroke-width":f, stroke:d.connectorColor || "#606060", visibility:da, zIndex:3}).translate(c.plotLeft, c.plotTop).add();
						}
					}
				}
			}
	...
	cb.Highcharts = {Chart:Gd, dateFormat:Nc, pathAnim:$c, getOptions:function () {
		return Wa;
	}, numberFormat:zd, Point:Ac, Color:Yb, Renderer:Wc, seriesTypes:vb, setOptions:function (a) {
		Wa = va(Wa, a);
		Bd();
		return Wa;
