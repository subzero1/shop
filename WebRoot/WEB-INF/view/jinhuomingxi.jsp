...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en">
	...
	<form id="searchForm" method="post" action="jinhuomingxi.do">
	<input type="hidden" name="jhdh" value="${param.jhdh }" />
	...
	<input type="hidden" id="pageRowSize" name="pageRowSize" value="${pageRowSize }"/>
		...
		<table border="0" cellspacing="0" cellpadding="0" style="border-collapse:collapse;margin-right: auto;margin-left: auto" class="jinhuodantab nicetable">
						<caption><b>进货单明细</b><input type="button" value="返回进货单" onclick="javascript:window.location.href='jinhuodan.do'" style="float:right;"/></caption>
						<!-- 遍历列名 -->
							<tr>
								<c:forEach items="${jinhuomingxi_titleList}" var="title">
									<th style="width:${title.width }px;display:${title.display }">${title.title }</th>
								</c:forEach>
							</tr>
							<!-- 遍历行 -->
							<c:forEach items="${jinhuomingxi_list}" var="jinhuomingxi">
								<tr>
									<!-- 根据列头 遍历每个单元格-->
									<c:forEach items="${jinhuomingxi_titleList}" var="title">
										<td style="text-align: ${title.align };display:${title.display }">
										<c:choose>
										<c:when test="${title.pattern=='price'}"><fmt:formatNumber pattern="￥0.00" value="${jinhuomingxi[title.key] }"/></c:when>
										<c:when test="${not empty title.pattern && title.pattern!='price'}"><fmt:formatDate pattern="${title.pattern}" value="${jinhuomingxi[title.key] }"/></c:when>
										<c:otherwise>${jinhuomingxi[title.key] }</c:otherwise>
										</c:choose>
										</td>
									</c:forEach>
								</tr>
